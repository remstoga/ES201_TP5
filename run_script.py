...
for nb_threads in nb_threads_l:
    for width in width_l:
        out = subprocess.run('$GEM5/build/ARM/gem5.fast $GEM5/configs/example/se.py --cpu-type=detailed --caches ' + '-w '+str(width)+' -n ' + str(nb_threads) + ' -c test_omp -o "' + str(nb_threads) +' 64"',shell=True)
        with open('m5out/stats.txt','r') as stats:
            for num_cpu in range(nb_threads):
                for ligne in stats:
                    if "number of cpu cycles simulated" in ligne:
                        results.write("nombre de threads : " + str(nb_threads) + ' nombre de voies : ' + str(width)+  ' ' + ligne)
